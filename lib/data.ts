...
export async function getAdminDashboard() {
  ...
  const [monthReferrals, monthThankYou, monthVisitors, monthOneToOnes, monthTestimonials, monthIntroductions, yearReferrals, yearThankYou, yearVisitors, yearOneToOnes, yearTestimonials, yearIntroductions] = await Promise.all([
    ...
    prisma.introduction.count({ where: { createdAt: { gte: monthStart } } }),
    ...
    prisma.introduction.count({ where: { createdAt: { gte: yearStart } } }),
  ]);
  ...
  return {
    ...
    metrics: {
      monthToDate: {
        ...
        introductions: monthIntroductions,
      },
      yearToDate: {
        ...
        introductions: yearIntroductions,
      },
    },
  };
}
  ...
  switch (type) {
    ...
    case "monthly_summary":
      return prisma.$queryRaw<
        Array<{ month: Date; referrals: bigint; thank_you: Prisma.Decimal | null; visitors: bigint; testimonials: bigint; introductions: bigint }>
      >`SELECT date_trunc('month', "createdAt") AS month,
          ...
          (SELECT COUNT(*)::bigint FROM "Introduction" i WHERE date_trunc('month', i."createdAt") = date_trunc('month', r."createdAt")) AS introductions
        FROM "Referral" r
